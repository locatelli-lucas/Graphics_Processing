...
void loadPlayerIdleTexture() {
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    int width, height, nrChannels;
    unsigned char* data = stbi_load("./assets/sprites/Idle.png", &width, &height, &nrChannels, 4);
    if (data) {
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, data);
        stbi_image_free(data);
    } else {
...
void drawPlayer(int i, int j, glm::mat4 projection) {
    ...
    float px        = screenX + winWidth / 2 - TILE_WIDTH / 2;
    float py        = screenY + offsetY;
    float spriteW   = TILE_WIDTH * 1.0f; 
    float spriteH   = TILE_HEIGHT * 2.0f; 
    glm::mat4 model = glm::mat4(1.0f);
    model = glm::translate(model, glm::vec3(px + (TILE_WIDTH - spriteW) / 2 + 15, py + (TILE_HEIGHT - spriteH) - TILE_HEIGHT / 4, 0.0f));
